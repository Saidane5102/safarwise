    ...
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <View style={styles.bottomNav}>
        {NAV_ITEMS.map((item, idx) => (
          ...
          >
            <View style={[styles.iconContainer, active === idx && styles.activeIconContainer]}>
              <Image
                source={item.icon}
                style={[styles.iconSvg, active === idx && styles.activeIconSvg]}
                resizeMode="contain"
              />
            </View>
            <Text style={[styles.navLabel, active === idx && styles.activeLabel]}>{item.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </View>
